...
void loop() {
  ...
  if (currentMillis - prevMillis >= LOOPTIME){
    ...
    speed_act_left = encoder0Diff/2.70;                    
    speed_act_right = encoder1Diff/2.70; 
  
    encoder0Error = (demand_speed_left*2.70)-encoder0Diff; // 3965 ticks in 1m = 39.65 ticks in 10ms, due to the 10 millis loop
    encoder1Error = (demand_speed_right*2.70)-encoder1Diff;
  
    encoder0Prev = encoder0Pos; // Saving values
    encoder1Prev = encoder1Pos;
  
    left_setpoint = demand_speed_left*2.70;  //Setting required speed as a mul/frac of 1 m/s
    right_setpoint = demand_speed_right*2.70;
